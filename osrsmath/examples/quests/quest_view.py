from osrsmath.examples.quests.quest import Player, QuestBook
...
class QuestViewer:
	...
	def assign_graph(self):
		...
		for quest in sum(self.shells, []):
			quest = self.player.quest_book[quest]
			...
			hover_details = '<br>'.join([
				...
				"<hr>",
				"<b>Requires:</b>", *["&emsp;"+' | '.join(prereqs) for prereqs in chunk(quest.quest_requirements, 2)],
				"<b>Progresses:</b>", *["&emsp;"+' | '.join(prereqs) for prereqs in chunk(self.player.quest_book.get_quests_requiring(quest.name), 2)],
				"<b>Needs:</b>", *["&emsp;"+' | '.join([f"{level} {skill}" for level, skill in prereqs]) for prereqs in chunk(quest.skill_requirements.items(), 2)],
				"<b>Rewards:</b>", *["&emsp;"+' | '.join([f"{xp:,} {skill} xp" for skill, xp in prereqs]) for prereqs in chunk(quest.rewards.items(), 2)],
			])
			self.graph.add_node(quest.name, title=hover_details, url=quest.url,
				shell=shell, tier=shell, difficulty=quest.difficulty, length=quest.length, series=quest.series,
	...
	def save_graph(self, file='quest_viewer.html', title="The Old School Runescape Universe", show=True):
		...
		contents.insert(contents.index("<head>\n")+1, '\n'.join([
			...
			'<link href="http://fonts.cdnfonts.com/css/runescape-uf" rel="stylesheet">',
			'<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>',
			'',
		]))
		contents.insert(contents.index("</center>\n"), 
			f'<h1><span style="font-family: RuneScape UF;color:yellow;">{title}</span> <a href="https://github.com/Palfore/OSRSmath" target="_blank" style="font-size:50%;">by&nbsp;OSRSmath&nbsp;-&nbsp;Palfore</a></h1>'+"""
			<div class="w3-row-padding">
				...
				<div class="w3-quarter">
					<select class="btn btn-primary" id="color_scheme" onchange="changeColor()" data-toggle="tooltip" title="Change the color scheme.">\n"""+
							'\n\t\t'.join(f"  <option value={option}>{option.replace('_', ' ').title()}</option>" for option in self.colors.keys()) + """
					</select>
				...
				<div class="w3-quarter">
                    ...
                    <button id="hide_all_quests" onclick=hide_all() class="btn btn-light" data-toggle="tooltip" title="Show or hide quests that are completed."></button>
				</div>
				<div class="w3-quarter">
		...
		contents.insert(contents.index('<style type="text/css">\n')+1, f"""
			div.vis-network {{background-color: {self.options['custom-background_color']};}}
			body {{background-color: {self.options['custom-background_color']};}}
		""")
		...
		contents.insert(contents.index("        network = new vis.Network(container, data, options);\n")+1, 
			open('quest_view.js').read().\
				...
				replace("BACKGROUND_COLOR", f'"{background_color}"').\
				replace("HELP", "'"+str(

class Spells:
	...
	@property
	def fire_spells(self):
		return [spell for spell in self.STANDARD if spell.startswith('fire')]

	@property
	def bolt_spells(self):
		return [spell for spell in self.STANDARD if spell.endswith('bolt')]

	@property
	def god_spells(self):
		return ['saradomin strike', 'claws of guthix', 'flames of zamorak']

	def _in(self, spell, list):
		if spell is None:
			return False
		return spell.lower() in list

	def requires_undead(self, spell):
		return self._in(spell, ['crumble undead'])

	def can_charge(self, spell, equipment):
		if spell is None:
			return None
		return {
			'saradomin strike': 'Saradomin cape' in equipment,
			'claws of guthix': 'Guthix cape' in equipment,
			'flames of zamorak': 'Zamorak cape' in equipment,
		}[spell.lower()]
		
	def is_fire(self, spell):
		return self._in(spell, self.fire_spells)

	def is_bolt(self, spell):
		return self._in(spell, self.bolt_spells)

	def is_standard(self, spell):
		return self._in(spell, self.STANDARD)

	def is_ancient(self, spell):
		return self._in(spell, self.ANCIENT)

	def get(self, name: str):
		if name is None:
			raise ValueError('Cannot get spell "None". Please supply a valid spell name.')
		name = name.lower().strip()
		try:
			return {**self.STANDARD, **self.ANCIENT}[name]
		except KeyError as e:
			print(f"The spell '{name}' could not be found in any spellbook.")
